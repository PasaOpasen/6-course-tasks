...
//      	}
  	...
  	for (unsigned int i = 499999999; i <= n; i++) // Первые два числа - это 0 и 1, пропускаем их и начинаем перебор с 2
	{
		if (a[i] == 0) // Пропускаем нули в массиве
		{
			continue;
		}
		if ( Ferma(a[i] + 10) == false) // Т.к. взятое a[i] + 0 - простое, то проверяем a[i] + 10 на простоту
		{
			continue;
		}
		if (Ferma(a[i] + 2) || Ferma(a[i] + 4) || Ferma(a[i] + 6) || Ferma(a[i] + 8) ) // Если a[i] + 0 и a[i] + 10 - простые, то проверяем что остальные числа составные
		{
			continue;
		}
